...
## Normal
seuratObject_Sample_Normal <- readRDS(file = "C:/Charlene/Code_GitHub_BioInport2025/KGD_CellTypeAnnot_Skin/Input/GSE202352_combined_seurat_integrated_BEC.rds")

seuratObject_Sample_Normal <- Seurat_Prepocessing(seuratObject_Sample_Normal)

DimPlot(seuratObject_Sample_Normal, group.by = "Tissue", reduction = "umap")
Layers(seuratObject_Sample_Normal)
...
seuratObject_Sample_Normal <- JoinLayers(seuratObject_Sample_Normal, assay = "RNA")

# # 在合併後的數據上運行 ScaleData
# seuratObject_Sample_Normal <- ScaleData(seuratObject_Sample_Normal)
...
seurat.list <- list(
  ...
  Normal = seuratObject_Sample_Normal
)
...
# 5. 進行批次效應校正並整合
seuratObject_Sample <- IntegrateData(anchorset = anchors)
...
seuratObject_Sample <- FindNeighbors(seuratObject_Sample, dims = 1:20) # dims可視需要調整
seuratObject_Sample <- FindClusters(seuratObject_Sample, resolution = 0.5)
seuratObject_Sample <- RunUMAP(seuratObject_Sample, dims = 1:20)
